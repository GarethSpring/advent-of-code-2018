...
namespace advent_of_code_2018.Days.Day21
{
    public class Solution
    {
        private const string Matcher = @"([a-z]{4})\s([0123456789]{1,8})\s([0123456789]{1,8})\s([0123456789]{1,8})";
        ...
        public int Run(int[] registers)
        {
            ...
            while (true)
            {
                Debug.WriteLine($"Executing: {instructions[instructionPointer]}");
                Debug.WriteLine($"IP is : {instructionPointer}");

                Execute(instructions[instructionPointer]);

                Debug.WriteLine($"Registers: {registers[0]},{registers[1]},{registers[2]},{registers[3]},{registers[4]},{registers[5]}");
        ...
        {
            File.ReadLines(@"days\day21\input\input.txt").ToList().ForEach(line =>
            {
                if (line.Contains("#ip"))
                {
